...
public class EnemyBoard : MonoBehaviour
{
    ...
    private void SetUp()
    {
        ...
        for (int x = 0; x < width; x++)
            for (int y = 0; y < height; y++)
            {
                ...
                backgroundTile.transform.parent = this.transform;//Âîçìîæíî òóò òî÷êà îòñ÷åòà
    ...
    //    {
    //        Debug.Log($"SwitchAndCheck  âîøåë {direction}");
    ...
    public bool SwitchAndCheck(int column, int row, Vector2 direction)
    {
        ...
        {
            Debug.Log($"SwitchAndCheck ÏÐÎÂÅÐÊÓ ÏÐÎØÅË ");
            SwitchPieces(column, row, direction);
            if (CheckForMatches())
            {
                SwitchPieces(column, row, direction);
        ...
        Debug.Log("SwitchAndCheck ÏÐÎÂÅÐÊÓ ÍÅÅÅÅÅÅÅÅÅÅÅ ÏÐÎØÅË");
        return false;
    }
